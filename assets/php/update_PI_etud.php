...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['id'])) {
    ...
    if (preg_match('/\s/', $new_identifiant)) {
        ...
        // Update the etudiants table first
        $etudiants_query = "UPDATE etudiants SET CNE = ? WHERE CNE = ?";
        $etud_stmt = $conn->prepare($etudiants_query);
        $etud_stmt->bind_param("ss", $new_identifiant, $id);

        if ($etud_stmt->execute()) {
            $etud_stmt->close();
            ...
            $_SESSION['message'] = "Une erreur est survenue lors de la mise à jour des informations d'etudient.";
            $_SESSION['status'] = "error";
            $etud_stmt->close();
        }
    }
    ...
    $conn->close();
    header("Location: ../admin/profile-Etud.php?id=" . urlencode($new_identifiant));
    exit();
} else {
    header("Location: ../admin/profile-Etud.php?id=" . urlencode($new_identifiant));
    exit();
